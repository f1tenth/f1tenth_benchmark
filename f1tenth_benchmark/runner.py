...
class Runner:
    ...
    def run(self, n_episodes: int) -> None:
        ...
        while n_episodes > 0:
            ...
            sim.add(state=state, action=None, info=None)

            while not scene.is_done():
                action, agent_info = self._agent(observation=state, agent_state=agent_state)
                state, scene_info = scene.step(action)
                sim.add(state=state, action=action, agent_info=agent_info, scene_info=scene_info)

            metrics = monitor.evaluate(sim)

            if self._log_dir is not None:
                sim.save(f"episode_{n_episodes}_sim.npz")
                np.savez_compressed(f"{self._log_dir}/episode_{n_episodes}_metrics.npz", **metrics)
